...
function afficherBillets(array $billets): void
{
    ...
    foreach ($billets as $billet):
        ...
        <article class="billet-card">
            ...
            <p><?= h($billet['date_evenement']) ?> à <?= h(substr($billet['heure_evenement'], 0, 5)) ?> — <?= h($billet['lieu']) ?></p>
            <p>Catégorie : <?= h($billet['categorie']) ?> — Association : <?= h($billet['association']) ?></p>
            <p>Statut : <?= h($billet['statut_inscription']) ?> — Paiement : <?= h($billet['paiement_statut']) ?></p>
